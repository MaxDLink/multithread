...
if(isLinkerError == 1) {
    ...
    std::ofstream outputFile("Data/parsed-errors.json");
...
void ParsingJob::JobCompletedCallback(){//TODO - report errors and warnings back to main thread. Main thread should then pass these to Parsingjob and OutputJob.
    ...
    //read from loopHit.txt file to set loopHit to 1
    std::ifstream file("./Data/loopHit.txt"); //open file
    if(!file){
        std::cout << "ERROR: FILE NOT FOUND" << std::endl;
    }
    std::string loopHitData;
    std::getline(file, loopHitData); //get each line from file
    file.close(); //close file
    bool loopHit = std::stoi(loopHitData); //convert string to int
    if(loopHit == 1){//TODO - only runs parsingJob loop when we are in the loop function inside of interpreter. 
        loopParsingJob(); 
    }
    //notify completion
...
void ParsingJob::loopParsingJob(){

    //create an integer set equal to 1 
    int loopCond = 1;
    //read from loopCond.txt file
    std::ifstream file("./Data/parsingJobLoopCounter.txt"); //open file
    if(!file){
        std::cout << "ERROR: FILE NOT FOUND" << std::endl;
    }
    std::string pastCounterData;
    std::getline(file, pastCounterData); //get each line from file
    file.close(); //close file
    int pastInt = std::stoi(pastCounterData); //convert string to int
    loopCond += pastInt; //add past int to loopCond
    std::ofstream file2("./Data/parsingJobLoopCounter.txt"); //open file to write to
    file2 << loopCond << std::endl; //write loopCond val to file
    file2.close(); //close file
    //std::cout << "ParsingJob on loop: " << loopCond << std::endl; //TODO - not reset??? 
    //if loopCond = 2, then we have looped once, and need to write val to file customjob to use as boolean value 
    if(loopCond == 3){//TODO - fix this to loop exactly as many times as the number they are equal to. Loopbool.txt main driver of this behavior! 
        //write to loopbool.txt 
        std::ofstream file3("./Data/loopbool.txt"); //open file to write to
        file3 << "1" << std::endl; //write 0 to file
