...
Job* MyJobSystemAPI::getLLMJob(){
    //std::cout << "The job types are compilejob, parsingjob, outputjob, customjob" << std::endl; 
    //TODO - add in ability to grab specific job later! For now just gives customJob LLM so that IP and Prompt can be passed to LLM job easily 
    return jobsForAPI["LLM"]; //return job type of custom job
...
int MyJobSystemAPI::getJobStatus(const std::string& jobType){
    ...
    if(jobType == "compilejob"){
        ...
        jobStatus = jsForAPI->GetJobStatus(job->GetUniqueID()); 
    }
    else if(jobType == "parsingjob"){
