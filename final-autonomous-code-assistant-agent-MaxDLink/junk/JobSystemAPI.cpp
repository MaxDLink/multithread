...
void MyJobSystemAPI::createJob(const std::string& jobType){
    if(jobType == "compilejob"){
                ...
                //TODO - [OLD INSTANTIATION] CompileJob* cjb = new CompileJob(0xFFFFFFFF, 1, directoryToMakefile); //pass in directory to makefile to run make command in cjb 
                //create a json object with properties for compile job
                json jsonInput;
                jsonInput["threadName"] = "Thread1"; //TODO - hardcoded ID needs to change 
