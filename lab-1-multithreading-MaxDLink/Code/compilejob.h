...
class CompileJob : public Job{
    private: 
    std::queue<std::string> & streamCjb; 
    std::string makefile; //string for grabbing makefile path 
    public: 
    CompileJob(unsigned long jobChannels, int jobType, std::queue<std::string> & stream, const std::string & passedInMakeFile): Job(jobChannels, jobType),streamCjb(stream), makefile(passedInMakeFile) {}; //constructor - TODO - added passing jobs vector to constructor to add parsing job to jobs vector
    ~CompileJob() {};
